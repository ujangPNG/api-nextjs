...
const GraphingCalculator: React.FC = () => {
  ...
  const generatePoints = (expression: string): [number[], number[]] => {
    ...
    return [
      points.map(p => p.x), // x values
      points.map(p => p.y)  // y values
    ];
  };
  ...
  const handleAddEquation = (): void => {
    ...
    try {
      ...
      setError('Invalid equation: ' + (err.message || 'Unknown error'));
    }
  };
  ...
  // Update equation
  const handleUpdateEquation = (id: number, newExpression: string): void => {
